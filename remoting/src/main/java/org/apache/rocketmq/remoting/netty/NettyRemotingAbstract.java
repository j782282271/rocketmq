...
/**
 * 发送信号量设置
 * 待返回的id->ResponseFuture表
 * processor表
 * channel事件存储队列
 * 作为server处理分发请求：req res分发：processResponseCommand、processRequestCommand
 * 作为client发送请求
 */
public abstract class NettyRemotingAbstract {
     ...
     * 每个processor都配置了自己的executor,异步处理
     *
     * @param ctx channel handler context.
    ...
    public void processResponseCommand(ChannelHandlerContext ctx, RemotingCommand cmd) {
        ...
        if (responseFuture != null) {
            ...
            //如果callback不为空，说明是回调，使用回调executor异步调用回调函数
     ...
     * channel close会调用本方法,通知所有等待结果的ResponseFuture失败
     *
