...
public class PullMessageProcessor implements NettyRequestProcessor {
    ...
    /**
     * 1broker是否允许读判断
     * 2consumerGroup订阅组配置信息获取
     * 3是否允许consumer
     * 4是否有TopicConfig、topic是否可读
     * 5队列id是否在允许范围
     * 6解析：是否long polling、是否commitOffset、是否使用subExpress：tag或者SQL92、long polling 时长
     * 7如果使用subExpress（tag或者SQL92(property filter)）则解析他们的过滤表达式
     * 8如果不使用subExpress，则根据topic获取ConsumerGroupInfo，对比ConsumerGroupInfo与subscriptionGroupConfig
     * ***获取subscriptionData，对比subscriptionData与请求中的订阅版本，如果小于则不允许拉消息，如果subscriptionData
     * ***有sql92的过滤配置则，同样对比版本
     * 9如果请求中使用sql92过滤但是服务端不允许则报错
     * 10根据broker端配置是否允许retry消息创建不同的MessageFilter的子类
     * 11从MessageStore根据messggeFilter过滤消息
     * 12得到消息后构建responseHeader
     * 13如果本broker为slave，且允许读则返回code：PULL_RETRY_IMMEDIATELY，并且提示client从master消费
     * 14如果有消息，根据broker配置参数：transferMsgByHeap，确定是通过heap发送给client还是直接directBuffer发送给client
     * 15如果没有消息，则调用getPullRequestHoldService，夯住一会儿请求
     * 16如果是offset_moved，则发送一条offset_moved消息
     * 17如果允许commitOffset则commitOffset
     */
    private RemotingCommand processRequest(final Channel channel, RemotingCommand request, boolean brokerAllowSuspend)
        ...
        if (getMessageResult != null) {
            ...
            if (this.brokerController.getBrokerConfig().isSlaveReadEnable()) {
                // 这个slave broker消费也很慢，提示从另一台slave broker消费
                if (getMessageResult.isSuggestPullingFromSlave()) {
                    responseHeader.setSuggestWhichBrokerId(subscriptionGroupConfig.getWhichBrokerWhenConsumeSlowly());
                }
                // consume ok
            ...
            switch (response.getCode()) {
                case ResponseCode.SUCCESS:
                    ...
                    if (this.brokerController.getBrokerConfig().isTransferMsgByHeap()) {
                        ...
                        try {
                            //通过堆，直接directBuffer转发msgBody到channel
    ...
    /**
     * 将getMessageResult中directBuffer中的消息复制到heap中
     * 统计存储到读取的时间差
     */
    ...
    /**
     * 构造OffsetMovedEvent的MessageExtBrokerInner存储到磁盘
     */
